...
class StudentStudent(models.Model):
    ...
    def create(self, vals):
        ...
        vals['pid'] = self.env['ir.sequence'].next_by_code('student.student')
    ...
    pid = fields.Char('Student ID', index=True, readonly=True, help='Personal IDentification Number')
            ...
            if rec.age <= 5:
                raise except_orm(_('Warning'),
                                 _('''The student is not eligible.
                                   Age is not valid.'''))
            domain = [('id', '=', rec.standard_id.id)]
            print "REC STANDARD ID IS ===="+str(rec.standard_id.id)

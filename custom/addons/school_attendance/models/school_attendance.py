...
class DailyAttendance(models.Model):
    ...
    user_id = fields.Many2one('hr.employee', 'Faculty',
                              help="Select Teacher",
                              states={'validate': [('readonly', True)]})
    # Add Timetable relation :
    class_session_id = fields.Many2one('time.table.line', 'Class Session', required=True)
    ...
    _sql_constraints = [
        ('attend_unique', 'unique(standard_id,user_id,date,class_session_id)',
         'Attendance should be unique!')
    ]

    @api.multi
    @api.onchange('class_session_id')
    def onchange_class_session(self):
        '''Method to make present false when student is absent'''
        self.start_time = self.class_session_id.start_time
        self.end_time = self.class_session_id.end_time
